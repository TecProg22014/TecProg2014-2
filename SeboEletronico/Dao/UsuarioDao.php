...
class UsuarioDao {
	...
	public function insertUser( $usuario ){
		...
		$passwordId = insertPassword($auxiliaryPassword[0]);
	...
	public function insertPassword( $password ){
		$insertUserPassword = "INSERT INTO senha (codigo_senha) VALUES ('".$password."')";
		mysql_query( $insertUserPassword );
		$searchUserByPassword = "SELECT id_senha FROM senha WHERE codigo_senha='".$finalPassword."'";
		//#REFACTOR: Verify double iquals of variable resultSearchUserByPassword and $passwordId in next line
	...
	public function deleteUser( $userEmail, $userPassword ){
		...
		//#REFACTOR: Implements on database CASCADE on DELETE on table 'senha'
